import { TYPES, CLASSES, ALL_TYPES } from "./store.js";
...
export class Parser {
    ...
    parse(args) {
        ...
        for (let pointer = 0; pointer < args.length; pointer++) {
            ...
            switch (arg) {
                ...
                case "-t":
                case "--type":
                    this.options.types = this.requireValue(
                        pointer,
                        "-t/--type [value(s)]"
                    );
                    pointer++;
    ...
    validate(options, log) {
        if (options.verbose && options.silent) {
            cliFail(
                "-v/--verbose and -q/--quiet/--silent cannot be used togethere"
            );
        }

        if (this.options.allType) {
