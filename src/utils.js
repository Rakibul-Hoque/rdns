      ...
      ? {
          ...
          raw: {
            buffer: t.response?.buffer
              ...
              : null,
            length: t.response?.buffer ? t.response.buffer.length : null,
          },
          decoded: {
            header: t.response.header,
            questions:
              t.response.questions.length === 0 ? null : t.response.questions,
            answers:
              t.response.answers.length === 0 ? null : t.response.answers,
            authority:
              t.response.authority.length === 0 ? null : t.response.authority,
            additional:
...
export function createTimeout(options, client, domains, trxid, log) {
  return setTimeout(() => {
    log.error(
      `DNS request timed out for ${domains.join(", ")} ` + `(ID: ${trxid})`,
    );
