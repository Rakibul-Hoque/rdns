...
import { fail, setConf, allTransactionsFinished, serializeJson } from "./utils.js";
...
export function cleanUp(options, log) {
  if (options.json) {
    ...
    if (options.json_export) {
      try {
        fs.writeFileSync(options.json_export, JSON.stringify(json, null, 2))
        log.info(
          `JSON output written to -> ${options.json_export}`
        );
