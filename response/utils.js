...
export class ResponseUtil {
    ...
    switch (answer.type) {
        case 1:
            return `${answer.name} A -> ${answer.data.address} TTL=${answer.ttl}`;

        case 2:
            return `${answer.name} NS -> ${answer.data.nameserver} TTL=${answer.ttl}`;

        case 5:
            return `${answer.name} CNAME -> ${answer.data.target} TTL=${answer.ttl}`;

        case 6:
            return `${answer.name} SOA -> ${answer.data.mname} ${answer.data.rname} serial=${answer.data.serial} TTL=${answer.ttl}`;

        case 12:
            return `${answer.name} PTR -> ${answer.data.target} TTL=${answer.ttl}`;

        case 15:
            return `${answer.name} MX -> ${answer.data.exchange} preference=${answer.data.preference} TTL=${answer.ttl}`;

        case 16:
            return `${answer.name} TXT -> ${answer.data.strings
                .map(s => `"${s}"`)
                .join(" ")} TTL=${answer.ttl}`;

        case 28:
            return `${answer.name} AAAA -> ${answer.data.address} TTL=${answer.ttl}`;

        case 33:
            return `${answer.name} SRV -> ${answer.data.target}:${answer.data.port} priority=${answer.data.priority} weight=${answer.data.weight} TTL=${answer.ttl}`;

        default:
            return `${answer.name} ${answer.typeName} -> ${answer.data?.raw ?? "<unknown>"} TTL=${answer.ttl}`;
    }
}
}
...
function formatAnswer(answer) {
    ...
    switch (answer.typeName) {
        ...
        case "TXT":
            value = answer.data.strings.map(str => `"${str}"`).join(" ");
            break;

        case "SRV":
            value =
                `${answer.data.target}:${answer.data.port} ` +
        ...
        default:
            value = answer.data?.raw ?? "<no data>";
    }

    return (
        `${answer.name} ` +
        `${answer.typeName} -> ${value} ` +
        `TTL=${answer.ttl}`
    );
}
